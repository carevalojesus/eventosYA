...
fun EventosYATheme(
    ...
    val colorScheme = when {
        dynamicColor && contrastLevel == ContrastLevel.Default && Build.VERSION.SDK_INT >= Build.VERSION_CODES.S -> {
            val context = LocalContext.current
            if (darkTheme) dynamicDarkColorScheme(context) else dynamicLightColorScheme(context)
        }

        contrastLevel == ContrastLevel.High -> {
            if (darkTheme) darkHighContrastScheme else lightHighContrastScheme
        }
